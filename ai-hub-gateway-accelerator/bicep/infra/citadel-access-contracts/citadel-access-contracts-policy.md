...
### Per-User / Per-Team Dynamic Sub-Quota Policy *(fork addition)*

Every `llm-token-limit` above is keyed on `context.Subscription.Id` — one
shared bucket per access contract, no per-individual-user or per-team
limit, and no self-service path if a scope runs out. `guides/quota-override-approval.md`
designs and `src/quota-service/` implements a **second, opt-in tier**
underneath the contract's own quota (which stays exactly as documented
above — a hard backstop the new tier can never exceed): a dynamic
sub-quota resolved per `oid`/department JWT claim, with a self-service
request → approve → take-effect workflow (no APIM redeploy, effective
within one APIM cache TTL window).

Opt in by adding four lines to a product policy's `inbound` section,
after your existing tier-1 `llm-token-limit`:

```xml
<set-variable name="quotaBaselineQuota" value="100000" /> <!-- match your tier-1 token-quota above -->
<include-fragment fragment-id="resolve-quota-scope" />
<include-fragment fragment-id="load-quota-allowance" />
<llm-token-limit counter-key="@((string)context.Variables["quotaScopeCacheKey"])"
    tokens-per-minute="@((string)context.Variables["quotaTpmTier"] == "elevated" ? 5000 : 1000)"
    estimate-prompt-tokens="false"
    token-quota="@((int)context.Variables["quotaEffectiveQuota"])"
    token-quota-period="Monthly" />
```

Requires `jwtRequired="true"` on the contract (see [JWT Authentication
Policy](#jwt-authentication-policy) above) — without a validated JWT
there's no `oid`/`department` claim to resolve a scope from, and this
degrades to a no-op (tier 1 alone still applies), not a failure. Full
design, data model, and guardrails: `guides/quota-override-approval.md`.
Backend service: `src/quota-service/README.md`.
