...
$gatewayAppRoles = @(
    ...
    @{
        # Fork addition — see guides/quota-override-approval.md and
        # bicep/infra/modules/quota-service/quota-api-policy.xml.
        # Required to call the /decide operation on the Quota Override
        # API (approve/deny a quota-override request). Assign this role
        # to whoever should be able to approve quota increases — NOT
        # automatically to anyone with Task.ReadWrite. Note the scope
        # this closes: "is this a verified role-holder", not "is this
        # the right approver for this specific department" — the latter
        # still needs the approver-resolution reference table flagged
        # elsewhere in this fork as a separate, unbuilt gap.
        allowedMemberTypes = @("User", "Application")
        description        = "Approve or deny quota-override requests via the Quota Override API"
        displayName        = "Quota.Approve"
        isEnabled          = $true
...
if ($existingApp -and $existingApp.Count -gt 0) {
    ...
    $appPayload = @{
        ...
        appRoles = @(
            ...
            @{
                # Fork addition — see the matching entry (and its full
                # rationale/scope comment) in $gatewayAppRoles above.
                allowedMemberTypes = @("User", "Application")
                description        = "Approve or deny quota-override requests via the Quota Override API"
                displayName        = "Quota.Approve"
                isEnabled          = $true
