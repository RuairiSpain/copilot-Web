...
| [**🆕 Power BI Dashboard**](./guides/power-bi-dashboard.md) | Usage analytics and cost allocation dashboards, including org / cost-center / individual-user pages and RLS roles. The paste-in measures/columns/RLS-role kit itself lives at [`powerbi-report-kit/`](./powerbi-report-kit/) — 22 files, 15-30 minutes of Power BI Desktop work on top of your existing `.pbix`, see that folder's own `00-README.md` |
| [**🔧 Cost Attribution Guide**](./guides/cost-attribution-guide.md) | *(fork addition)* Per-record cost baked in at ingestion via a versioned, daily-refreshed pricing service — no more live price join, no retroactive drift on closed billing periods |
| [**📐 Advanced PTU/PAYG Routing (design only)**](./guides/advanced-ptu-routing.md) | *(fork addition)* Capacity-aware PTU/PAYG routing, documented as a future Phase 3 feature — **not implemented**; today's reporting covers the recommendation, not automated routing |
| [**🧵 Agent Trace Instrumentation Checklist**](./guides/agent-trace-instrumentation-checklist.md) | *(fork addition)* Why workflow/agent bottleneck visibility is a developer-side OpenTelemetry instrumentation task, not a Power BI report — points at Application Insights' Application Map, which already does this once an agent propagates trace context |
| [**🌳 Agent Hierarchy Attribution**](./guides/agent-hierarchy-attribution.md) | *(fork addition)* Sub-agent/MCP-tool call-chain attribution — propagated `x-agent-root-id`/`caller-type`/`depth` headers with OBO-vs-self-asserted trust tiering, correlated into `llm-usage-container` via Application Insights' own `operation_Id` (`frag-resolve-agent-hierarchy.xml`), plus a depth guard (log-only by default) and an always-enforcing fan-out guard (`frag-enforce-agent-limits.xml`), both reusing this accelerator's existing Redis-backed APIM quota primitives rather than a new subsystem. Power BI roll-up/drill-down measures in `powerbi-report-kit/23-agent-hierarchy-measures.dax`. **Still design-only**: the OBO token-exchange mechanics themselves (needs an Entra app registration this session can't provision — see `samples/agent-obo-client/`) and the atomic multi-scope quota decrement — see the guide's "What's actually built" |
| [**🎟️ Quota Override & Approval Workflow**](./guides/quota-override-approval.md) | *(fork addition)* Per-user/team dynamic sub-quota underneath the unchanged contract ceiling, self-service request/approve, JWT + `Quota.Approve`-role-gated authorization, fail-open cache-backed enforcement — `src/quota-service/` (built, 164 unit tests passing, 100% line coverage on every `src/` file — see the guide's coverage note), independent bearer-token re-validation closing the direct-call-bypass hole (`tokenValidation.ts`, fail-closed), SMTP email notifications on both request-created and decided, `quota-api.bicep` (the auth gateway), and a Power Apps write-back kit (`power-platform/quota-connector/`) embeddable directly in Power BI report pages. Team-scoped requests are now department-gated to the right approver (claim-based, unit-tested); individual-user-scoped requests remain open to any `Quota.Approve` holder — see the guide's "Implementation status" |
| [**🛡️ Enterprise Hardening Checklist**](./guides/enterprise-hardening-checklist.md) | *(fork addition)* Full-platform security review (base accelerator + every fork addition) across identity, network isolation, secrets, data protection, resiliency, audit, and abuse guards — three-column format (what exists / what changed / residual risk), including the function-key-vs-managed-identity gap the new Function Apps inherit and haven't yet closed |
