...
ORDER BY c._ts DESC
```
### 💰 Cost Attribution & Quota Governance *(fork addition)*

Two things this fork adds on top of the base accelerator, worth knowing
about even for a non-production deployment:

**Every usage record now has a real dollar cost on it.** You don't need
to do anything to turn this on — it's built into the ingestion pipeline.
Open the Power BI report kit (`guides/power-bi-dashboard.md`) and you'll
see actual `$` figures per model/product/team, not just token counts.
Behind the scenes: a small extra service (`src/pricing-service/`) prices
each record using the price that was actually in effect when the request
happened, so a later price change never silently changes what an old
request "cost." Full story: `guides/cost-attribution-guide.md`.

**If a team or person runs out of their monthly token allowance, there's
now a way to ask for more without you editing a Bicep file.** By default,
every access contract has one shared quota (unchanged from how this
accelerator always worked). This fork adds an *optional* second layer — a
per-user or per-team quota underneath that — plus a request/approve flow:
someone submits a request, an approver says yes or no, and if approved it
takes effect within a few minutes, no redeployment. This is **off by
default** — you opt a contract into it by adding four lines to its policy
file (see `bicep/infra/citadel-access-contracts/citadel-access-contracts-policy.md`,
"Per-User / Per-Team Dynamic Sub-Quota Policy"). If you never add those
four lines, nothing about your deployment changes. Full story:
`guides/quota-override-approval.md`.

**Neither of these is deployed by `azd up` automatically** — both
`pricing-service` and `quota-service` are small, standalone Function Apps
you deploy as one extra step after your quick deployment finishes (each
has its own `bicep/infra/modules/.../*.bicep` module and its own README
with the exact commands). If you skip that step, your quick deployment
still works exactly as it always has — usage records just won't have a
`cost` field yet, and the per-user/team quota fragments (if you opted a
contract into them) will fail open to the contract's existing quota with
no per-user/team enforcement, per their own fail-open design.
