...
Despite the name, this is a generic `Microsoft.Web/serverFarms`-delegated
subnet, not something specific to Logic Apps — this fork's two Function
Apps (`pricing-service`, `quota-service`) use the exact same
delegation/subnet shape when their optional `functionAppSubnetId` param
is set (`guides/enterprise-hardening-checklist.md` §2), so size this
subnet for all of them sharing it, not just the Logic App:
