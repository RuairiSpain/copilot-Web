...
- **Control costs** with usage attribution, quotas, and chargeback — including exact, per-request dollar cost attribution *(fork addition)*, priced at the rate in effect at the time of the request so a later price change never retroactively changes a closed period's numbers
- **Demonstrate governance** with audit-ready transparency and reporting
- **Cut the administrative overhead of quota management** *(fork addition)* — a self-service request/approval workflow for team- and individual-level quota increases means fewer support tickets and no waiting on an engineering deployment cycle to unblock a team, while every request and decision remains a permanent, immutable audit record and the platform's own overall spend ceiling per use case never changes without your explicit, reviewed sign-off
...
- Usage analytics and cost management, including per-request cost attribution with a versioned, daily-refreshed price catalog *(fork addition — `guides/cost-attribution-guide.md`)*
...
- Self-service quota request/approval workflow *(fork addition, backend built — `guides/quota-override-approval.md`, `src/quota-service/`)*: the enforcement mechanism, data model, and 5-minute-cycle notification pipeline are implemented and unit-tested; still needed before this is usable end-to-end are the actual request/approve front-end (a form, bot, or admin page) and the approver-resolution reference data
- Agent/sub-agent hierarchy token attribution *(fork addition, design + OBO code samples only — `guides/agent-hierarchy-attribution.md`, `samples/agent-obo-client/`)*: not yet a shipped capability, propagation itself isn't implemented in the gateway policy
