...
| `decideQuotaRequest` authorization | N/A (new) | **Fixed, in three layers now.** `bicep/infra/modules/quota-service/quota-api.bicep` gates `/decide` and `/pending` behind a validated JWT + the `Quota.Approve` role and stamps verified `oid`/`department` onto the request; `tokenValidation.ts`/`requestAuth.ts` independently re-validate the caller's real bearer token and cross-check both claims against those headers (`identityMatchesToken`, `department` mismatch check, both unit-tested) before any are trusted; `decideQuotaRequest.ts` then rejects (403) a `team`-scoped decision whose `scopeId` doesn't equal the approver's own verified department (`approverAuthorizedForScope()`, unit-tested) — closing both the direct-call-with-just-the-function-key bypass AND (for team-scoped requests) the "any role holder can decide any request" gap this row used to flag as still open. | **What's still open, narrowed but not eliminated**: `user`-scoped requests remain all-or-nothing — any `Quota.Approve` holder can still decide one, since there's no org-hierarchy/manager data source in this fork to resolve "the right approver" for an individual (a deliberate scope choice, not an oversight — see guides/quota-override-approval.md). The Approve/Deny canvas app screen still shows every pending *individually-scoped* request to every role holder; team-scoped ones are now actually gated. `tokenValidation.ts` itself is also unverified against a live Entra tenant — real security logic that has not been exercised against a real token, worth prioritizing if this goes to production. |
